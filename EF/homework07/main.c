...
#define CONTRACT_SIZE 5
#define EMPTY_LIMIT 16
#define MAX_CHAR_SIZE 82
...
int Enque(StringsQueue *q, char *x) {
    ...
    if (q->num >= q->max) {
        tmp = calloc(q->max + EXPAND_SIZE, sizeof(char *));
    ...
    q->num++;
    q->que[q->rear] = calloc(MAX_CHAR_SIZE, sizeof(char));
...
int Deque(StringsQueue *q, char *x) {
    ...
    if (q->max - q->num >= EMPTY_LIMIT) {
        tmp = calloc(q->max - CONTRACT_SIZE, sizeof(char *));
...
int main(void) {
    ...
    while (1) {
        int m, count = 0;
        char *x = calloc(MAX_CHAR_SIZE, sizeof(char));
